 ...
 * @author Zapdos
 */
public class ViewBattle extends HttpServlet {
        ...
        try {
                Player pl = (Player)request.getSession().getAttribute("player");
                    CardBattle cb = new CardBattle();
                        LinkedList<String> BattleList = cb.getBattle(pl.getId());
                        request.setAttribute("BattleList",BattleList);
                            getServletContext().getRequestDispatcher("/ViewBattle.jsp").forward(request, response);
